...
const cssTargetCommentMarker = 'Better Text SCSS targets';
...
export const SourceEditorField: React.FunctionComponent<SourceEditorFieldProps> = (props) => {
  ...
  const [monacoDiagnostic, setMonacoDiagnostic] = React.useState<MonacoDiagnostic>(() =>
    createMonacoDiagnostic('loading')
  );
  ...
  return (
    <div className="bt-css-editor">
      ...
      {floatingOpen && renderFloatingEditorLayer(
        <div
        ...
        >
          ...
          <div className="bt-floating-editor__toolbar" aria-label="SCSS target shortcuts">
...
const MonacoDiagnosticNotice: React.FunctionComponent<MonacoDiagnosticNoticeProps> = (props) => {
  ...
  const message = diagnostic.status === 'error'
  ...
  return (
    <div className={`bt-css-editor__diagnostic bt-css-editor__diagnostic--${diagnostic.status}`}>
      <span>{message}</span>
      <button
        className="bt-css-editor__diagnostic-copy"
        type="button"
        onClick={() => copyMonacoDiagnostic(diagnostic)}
      >
        Copy diagnostics
      </button>
    </div>
...
const BundledMonacoEditor: React.FunctionComponent<BundledMonacoEditorProps> = (props) => {
  ...
  React.useEffect(() => {
    ...
    const modelUri = bundledMonaco.Uri.parse(`inmemory://better-text/${props.path}`);
...
function createMonacoDiagnostic(
  status: MonacoDiagnosticStatus,
  message?: string
): MonacoDiagnostic {
  return {
...
function publishMonacoDiagnostic(diagnostic: MonacoDiagnostic): void {
  ...
  (window as unknown as { __betterTextMonaco?: MonacoDiagnostic }).__betterTextMonaco = diagnostic;
}
...
function insertCssTarget(
  ...
  editor.focus?.();
  editor.pushUndoStop?.();
  editor.executeEdits?.('better-text-target-shortcut', [
    {
...
function registerBetterTextCompletions(monaco: any): void {
...
function createSelectorSuggestions(monaco: any, range: any, targets: SourceEditorTarget[]): any[] {
  ...
  return targets.map((target) => ({
    ...
    detail: target.selector === ':host' ? 'Better Text host selector' : 'Better Text selector',
    documentation: target.selector.startsWith('.bt-')
      ? 'Targets only this generated Better Text instance.'
      : 'Targets a Better Text element.',
...
function createPropertySuggestions(monaco: any, range: any): any[] {
  const snippetRule = monaco.languages.CompletionItemInsertTextRule.InsertAsSnippet;
  return [
    ['font-family', 'font-family: ${1:"Inter", system-ui, sans-serif};', 'Google font family with fallback stack'],
    ['line-height', 'line-height: ${1:1.4};', 'Leading (unitless multiplier)'],
    ['letter-spacing', 'letter-spacing: ${1:0px};', 'Letter spacing in px'],
    ['font-size', 'font-size: ${1:17px};', 'Base text size'],
    ['font-weight', 'font-weight: ${1|400,500,600,700|};', 'Text weight'],
    ['color', 'color: ${1:#242424};', 'Text color'],
    ['text-align', 'text-align: ${1|left,center,right,justify|};', 'Text alignment'],
    ['text-transform', 'text-transform: ${1|none,uppercase,lowercase,capitalize|};', 'Text casing'],
    ['max-width', 'max-width: ${1:70ch};', 'Measure (line length)']
...
function createValueSuggestions(monaco: any, range: any): any[] {
  return [
    ['1.4', 'Default leading'],
    ['normal', 'Reset leading or letter spacing'],
    ['0px', 'No letter spacing'],
    ['uppercase', 'Uppercase text'],
    ['center', 'Center text'],
    ['justify', 'Justified text'],
    ['#242424', 'Default text color'],
    ['#0078d4', 'SharePoint blue'],
    ['#c50f1f', 'SharePoint red'],
    ['70ch', 'Comfortable line length'],
    ['17px', 'Default SharePoint body size']
  ].map(([label, detail]) => ({
    label,
